...
self.onmessage = (e) => {
  ...
  try {
    const gen = evaluateDesign(payload);
    let r,
      last = -1;
    do {
      r = gen.next();
      if (!r.done && r.value?.progress != null && r.value.progress - last >= 0.01) {
        last = r.value.progress;
        self.postMessage({ id, type: 'progress', progress: last });
      }
    } while (!r.done);
