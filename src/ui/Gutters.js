...
export function installGutters(root, onResize) {
  ...
  for (const g of root.querySelectorAll('.gutter')) {
    const k = g.dataset.var;
    if (!k) continue;
    const sign = g.dataset.dir === 'right' ? -1 : 1,
      min = +g.dataset.min || 160,
      max = +g.dataset.max || 720,
      def = +g.dataset.default || 300;
    state[k] = clamp(+saved[k] || def, min, max);
    apply(k);
    g.addEventListener('pointerdown', (e) => {
      ...
      const x0 = e.clientX,
        w0 = state[k];
      const move = (ev) => {
        state[k] = clamp(w0 + sign * (ev.clientX - x0), min, max);
        apply(k);
        onResize?.();
      };
      const up = () => {
