export class Notify {
  constructor(el) {
    this.el = el;
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') this.closePopovers(); });
  }
  ...
  popover(title, body) {
    ...
    const t = document.createElement('div'); t.className = 'toast pop'; t.dataset.title = title;
    t.innerHTML = `<div class="pop-head"><h4></h4><button type="button" class="x" aria-label="close" title="close (Esc)">✕</button></div><pre></pre>`;
    t.querySelector('h4').textContent = title; t.querySelector('pre').textContent = body;
    t.querySelector('.pop-head').addEventListener('click', () => t.remove());
    this.el.appendChild(t); return t;
  }
  closePopovers() { for (const t of this.el.querySelectorAll('.toast.pop')) t.remove(); }
}