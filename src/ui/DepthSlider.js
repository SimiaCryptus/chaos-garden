...
export class DepthSlider {
  constructor(el, { params, bus, barrier }) {
    ...
    for (const k of ['H', 'Re', 'inflow', 'spanwise', 'walls', 'twin', 'tier', 'backend']) {
      const l = el.querySelector(`[data-k=${k}]`).closest('label, .H');
      if (l) l.title = SCHEMA[k].desc;
    }
    this.range = el.querySelector('[data-k=H]');
    this.out = el.querySelector('[data-v=H]');
    this.cv = el.querySelector('canvas');
    this.range.addEventListener('input', () => {
      this.out.value = toH(+this.range.value).toFixed(3);
      this.drawMarkers(toH(+this.range.value));
    });
    ...
    el.addEventListener('change', (e) => {
      ...
      if (!k || k === 'H') return;
      params.set(k, e.target.type === 'checkbox' ? e.target.checked : e.target.value);
    });
    el.addEventListener('click', (e) => {
      if (e.target.closest('[data-act=settings]')) bus.emit('ui:action', 'settings');
    });
    bus.on('params:change', () => this.sync());
  ...
  sync() {
    const p = this.params;
    this.range.value = toS(p.get('H'));
    this.out.value = p.get('H').toFixed(3);
    for (const k of ['Re', 'inflow', 'spanwise', 'walls', 'tier', 'backend'])
      this.el.querySelector(`[data-k=${k}]`).value = p.get(k);
    this.el.querySelector('[data-k=twin]').checked = p.get('twin');
    this.drawMarkers(p.get('H'));
  }
  refreshScales() {
    if (!this.grid) return;
    this.scales = this.barrier.geometricScales(this.grid.hy);
    this.drawMarkers(this.params.get('H'));
  }
  setInfo({ grid, solver, tierName }) {
    const Re = this.params.get('Re'),
      Reh = Re * grid.H;
    const r = solver.report;
    const res = r.residual != null ? ` · res ${r.residual.toExponential(1)}` : '';
    this.el.querySelector('[data-v=info]').textContent =
      `${TIERS[tierName]?.label || tierName} · ${solver.backend} · ${grid.Nx}×${grid.Ny}×${grid.Nz}${grid.quasi2D ? ' (quasi-2D floor)' : ''} · Re_h ${Reh.toFixed(0)} · k_h ${grid.kh.toFixed(1)} · Δt ${solver.dt.toExponential(2)} · ${r.diffusion} · p-iters ${r.pIters}/${solver.pIters}${res}`;
  }
  setSettled(settled, progress) {
    const b = this.el.querySelector('[data-v=settle]');
    b.textContent = settled ? 'measuring' : `settling ${(progress * 100).toFixed(0)}%`;
    b.classList.toggle('ok', settled);
  }
  ...
  drawMarkers(H) {
    ...
    const s = this.scales;
    const all = s ? [...s.gaps, ...s.obstacles].map((q) => q.scale / 2) : [];
    if (all.length) {
