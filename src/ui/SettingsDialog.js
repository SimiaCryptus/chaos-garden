...
const DERIVED = {
  nu: { label: 'Kinematic viscosity ν', desc: 'ν = 1/Re in units of the reference speed × Ly. How "thick" the water is — honey has a large ν, water a small one. Editing ν rewrites Re; the two describe the same fluid.' },
};
export class SettingsDialog {
  constructor({ params, notify, bus, barrier }) {
    this.params = params; this.notify = notify; this.bus = bus; this.barrier = barrier; this.grid = null; this.scales = null; this.info = '';
    const d = this.el = document.createElement('dialog'); d.className = 'settings'; document.body.appendChild(d);
    ...
    for (const g of GROUPS) { const sec = document.createElement('section'); if (g.wide) sec.className = 'wide'; sec.innerHTML = `<h3>${g.title}</h3>`; for (const k of g.keys) sec.appendChild(this._row(k)); body.appendChild(sec); }
    this.range = d.querySelector('[data-k=Hs]'); this.out = d.querySelector('[data-v=H]'); this.cv = d.querySelector('canvas.markers');
    this.range.addEventListener('input', () => { const H = toH(+this.range.value); this.out.value = H.toFixed(3); d.querySelector('[data-k=H]').value = H.toFixed(4); this.drawMarkers(H); });
    this.range.addEventListener('change', () => params.set('H', +toH(+this.range.value).toFixed(4)));
    body.addEventListener('change', (e) => { const k = e.target.dataset.k; if (k && k !== 'Hs') this._change(e.target); });
    d.querySelector('[data-act=defaults]').addEventListener('click', () => { const patch = {}; for (const k in SCHEMA) if (k !== 'tier' && k !== 'backend') patch[k] = SCHEMA[k].default; params.set(patch); this.sync(); });
    bus?.on('params:change', () => { if (d.open) this.sync(); });
    bus?.on('design:change', () => this.refreshScales());
  }
  _row(k) {
    const def = SCHEMA[k] || DERIVED[k], row = document.createElement('div'); row.className = 'row';
    if (k === 'H') {
    ...
    else if (def.type === 'number') ctl = `<input type="number" data-k="${k}" min="${def.min}" max="${def.max}" step="${def.step}">`;
    else if (def.type === 'enum') ctl = `<select data-k="${k}">${def.values.map((v) => `<option>${v}</option>`).join('')}</select>`;
  ...
  _change(t) {
    ...
    if (k === 'nu') { const v = +t.value; if (v > 0) this.params.set('Re', 1 / v); this.sync(); return; }
    const def = SCHEMA[k], v = def.type === 'boolean' ? t.checked : def.type === 'number' ? +t.value : t.value;
    this.params.set(k, v); const after = this.params.get(k);
    if (def.type === 'number' && Number.isFinite(v) && after !== v) this.notify.show(`${k}: ${v} clamped to ${after}`, { kind: 'warn' });
    this.sync();
  }
  sync() {
    const p = this.params;
    for (const el of this.el.querySelectorAll('[data-k]')) {
      ...
      if (k === 'Hs') el.value = toS(p.get('H')); else if (k === 'nu') el.value = (1 / p.get('Re')).toExponential(3);
      else if (el.type === 'checkbox') el.checked = p.get(k); else el.value = p.get(k);
    }
    this.out.value = p.get('H').toFixed(3);
    const U = Math.abs(p.get('flow')), Re = p.get('Re');
    this.el.querySelector('[data-v=derived]').textContent = `ν = ${(1 / Re).toExponential(2)} · Re_eff = |U₀|·Re = ${(U * Re).toFixed(0)} · Re_h = Re·H = ${(Re * p.get('H')).toFixed(0)} · flow ${p.get('flow') < 0 ? '← right to left' : p.get('flow') > 0 ? '→ left to right' : 'still'}${p.get('topology') === 'torus' ? ' · torus' : ''}`;
  ...
  refreshScales() { if (!this.grid) return; this.scales = this.barrier.geometricScales(this.grid.hy); if (this.el.open) this.drawMarkers(this.params.get('H')); }
  /** Live solver status line (built by App); only touches the DOM while the dialog is open. */
  setInfo(info) { this.info = info; if (this.el.open) this.el.querySelector('[data-v=info]').textContent = info; }
  /** Crossover: k_h = π/(H·Ly) equals k_ℓ = 2π/ℓ when H* = ℓ/(2·Ly). */
  drawMarkers(H) {
    const cv = this.cv, W = cv.clientWidth || 400, dpr = devicePixelRatio || 1;
    if (cv.width !== Math.round(W * dpr)) { cv.width = Math.round(W * dpr); cv.height = Math.round(18 * dpr); }
    const g = cv.getContext('2d'); g.setTransform(dpr, 0, 0, dpr, 0, 0); g.clearRect(0, 0, W, 18);
    const X = (h) => (Math.log10(h) - LG0) / (LG1 - LG0) * W;
    const s = this.scales, all = s ? [...s.gaps, ...s.obstacles].map((q) => q.scale / 2) : [];
  ...
  open() { if (!this.el.open) this.el.showModal(); this.sync(); }
}