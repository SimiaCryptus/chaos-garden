...
export class Toolbar {
  constructor(el, bus) {
    ...
    el.addEventListener('click', (e) => {
      ...
      if (b.dataset.mode) bus.emit('ui:mode', b.dataset.mode);
      else if (b.dataset.act) bus.emit('ui:action', b.dataset.act);
    });
  ...
  setRunning(r) {
    this.el.querySelector('[data-act=toggle]').textContent = r ? '❚❚' : '▶';
  }
  setScore(res, label) {
    this.badge.textContent = `S ${res.score.toFixed(1)}`;
    const small = document.createElement('small');
    small.textContent = res.flags.length ? res.flags.join(' ') : 'valid';
    this.badge.appendChild(small);
