...
export class LineChart {
  ...
  draw(series, opts = {}) {
    ...
    const tx = logx ? Math.log10 : (x) => x,
      ty = logy ? Math.log10 : (y) => y;
    const ok = (x, y) =>
      Number.isFinite(x) && Number.isFinite(y) && (!logx || x > 0) && (!logy || y > 0);
    let x0 = Infinity,
      x1 = -Infinity,
      y0 = Infinity,
      y1 = -Infinity;
    ...
    if (ymin != null && (!logy || ymin > 0)) y0 = ty(ymin);
    if (ymax != null && (!logy || ymax > 0)) y1 = ty(ymax);
    if (yzero && !logy && Number.isFinite(y0)) {
      y0 = Math.min(y0, 0);
      y1 = Math.max(y1, 0);
    }
    ...
    if (!Number.isFinite(x0) || !Number.isFinite(y0)) {
      g.textAlign = 'center';
      g.fillText('no data yet', W / 2, Hh / 2 + 4);
      return;
    }
    if (x1 - x0 < 1e-12) {
      x0 -= 1;
    ...
    const L = 6,
      ...
      pw = W - L - R,
      ph = Hh - T - B;
    const X = (x) => L + ((tx(x) - x0) / (x1 - x0)) * pw,
      Y = (y) => T + (1 - (ty(y) - y0) / (y1 - y0)) * ph;
    g.save();
    g.beginPath();
    g.rect(L, T, pw, ph);
    g.clip();
    for (const b of bands) {
      ...
      const a = X(b.x0),
        c = X(b.x1);
      g.fillRect(a, T, Math.max(1, c - a), ph);
    }
    for (const v of vlines) {
      if (!ok(v.x, 1)) continue;
      g.strokeStyle = v.color || '#ffb454';
      g.setLineDash([3, 3]);
      g.beginPath();
      g.moveTo(X(v.x), T);
      g.lineTo(X(v.x), T + ph);
      ...
      g.setLineDash([]);
      if (v.label) {
        g.fillStyle = v.color || '#ffb454';
        g.fillText(v.label, X(v.x) + 2, T + ph - 3);
      }
    }
    if (yzero && !logy) {
    ...
    g.fillStyle = '#8a93a3';
    g.textAlign = 'right';
    g.fillText(fmt(back(y1, logy)), W - 2, T + 9);
    g.fillText(fmt(back(y0, logy)), W - 2, T + ph - 2);
    g.fillText(fmt(back(x1, logx)), W - 2, Hh - 2);
    g.textAlign = 'left';
    g.fillText(fmt(back(x0, logx)), L, Hh - 2);
    let lx = W - 2;
    g.textAlign = 'right';
