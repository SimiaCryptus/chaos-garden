...
export class JobRunner {
  ...
  get concurrency() {
    return this.useWorker ? this.poolSize : 1;
  }
  run(payload, onProgress) {
    return new Promise((resolve, reject) => {
      const id = ++this.id,
        job = { id, payload, onProgress, resolve, reject, cancelled: false, slot: null };
      this.jobs.set(id, job);
      if (this.useWorker) {
        this.queue.push(job);
        this._dispatch();
      } else this._runInline(job);
    });
  }
  _spawn() {
    try {
      ...
      this._workersFailed(String(err));
      return null;
    }
  }
  _dispatch() {
    ...
    if (!this.useWorker) {
      ...
      for (const j of q) this._runInline(j);
    }
  }
  ...
  async _runInline(job) {
    ...
    let gen;
    try {
      gen = evaluateDesign(job.payload);
    } catch (err) {
    ...
    const pump = () => {
      ...
      try {
        do {
          r = gen.next();
          if (!r.done && r.value?.progress != null) job.onProgress?.(r.value.progress);
        } while (!r.done && performance.now() - t < 12);
      } catch (err) {
        this.jobs.delete(job.id);
        return job.reject(err);
      }
