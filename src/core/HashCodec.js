...
export function decodeMask(str) {
  const b = b64urlToBytes(str);
  let pos = 0,
    w,
    h,
    run;
  [w, pos] = readVarint(b, pos);
  [h, pos] = readVarint(b, pos);
  const mask = new Uint8Array(w * h);
  let n = 0,
    cur = 0;
  while (pos < b.length && n < mask.length) {
    [run, pos] = readVarint(b, pos);
    if (cur) mask.fill(1, n, Math.min(n + run, mask.length));
    n += run;
    cur ^= 1;
  }
  return { w, h, mask };
...
export function encodeParams(p) {
  const o = {};
  for (const [k, s] of Object.entries(SHORT_KEYS)) if (k in p) o[s] = p[k];
  return bytesToB64url(new TextEncoder().encode(JSON.stringify(o)));
}
export function decodeParams(str) {
  const o = JSON.parse(new TextDecoder().decode(b64urlToBytes(str)));
  const p = {};
  for (const [s, v] of Object.entries(o)) if (LONG_KEYS[s]) p[LONG_KEYS[s]] = v;
  return validate(p).normalized;
}
/** `#cg1.<params-b64>.<design-b64>` */
export function encodeState({ params, mask, w, h }) {
  return `cg1.${encodeParams(params)}.${encodeMask(mask, w, h)}`;
}
