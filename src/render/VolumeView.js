...
export class VolumeView {
  constructor(grid) {
    this.range = { vorticity: 1, speed: 1 };
    this.setGrid(grid);
  }
  setGrid(grid) {
    this.grid = grid;
    this.data = new Uint8Array(grid.N * 4);
    this.scalar = new Float32Array(grid.N);
  }
  compose(solver, layer = 'vorticity') {
    ...
    if (speed) {
      const { u, v, w } = solver;
      for (let n = 0; n < N; n++) {
        const a = Math.sqrt(u[n] * u[n] + v[n] * v[n] + w[n] * w[n]);
        f[n] = a;
        if (a > m) m = a;
      }
    } else {
    ...
    const r = this.range[key],
      L = speed ? lut('viridis') : lut('coolwarm'),
      dens = speed ? 0.35 : 1; // a plug flow is |u| ≈ U₀ everywhere; keep it see-through
    for (let n = 0, o = 0; n < N; n++, o += 4) {
      ...
      let a = x < 0 ? -x : x;
      if (a > 1) a = 1;
      let t = speed ? x : 0.5 + 0.5 * x;
      t = t < 0 ? 0 : t > 1 ? 1 : t;
      const i = Math.round(t * 255) * 3;
      d[o] = L[i];
      d[o + 1] = L[i + 1];
      d[o + 2] = L[i + 2];
      d[o + 3] = Math.round(250 * a * a * dens);
    }
    return d;
  }
  _dye(solver) {
    ...
    for (let n = 0, o = 0; n < N; n++, o += 4) {
      ...
      const s = 255 / tot,
        a = tot > 1 ? 1 : tot; // hue = flux-weighted band mix, density = how much dye is here
      d[o] = Math.min(255, r * s);
      d[o + 1] = Math.min(255, g * s);
      d[o + 2] = Math.min(255, b * s);
      d[o + 3] = Math.round(250 * Math.sqrt(a));
    }
    return d;
  }
}
