...
const stops = (pts) => (t) => {
  t = Math.max(0, Math.min(1, t)) * (pts.length - 1);
  const i = Math.min(pts.length - 2, Math.floor(t)),
    f = t - i;
  return [0, 1, 2].map((c) => pts[i][c] + f * (pts[i + 1][c] - pts[i][c]));
};
const cividis = stops([
  [0, 0.135, 0.304],
  [0.255, 0.263, 0.404],
...
export function lut(name) {
  ...
  const f = MAPS[name] || viridis,
    out = new Uint8ClampedArray(256 * 3);
  for (let i = 0; i < 256; i++) {
    const c = f(i / 255);
    out[i * 3] = c[0] * 255;
    out[i * 3 + 1] = c[1] * 255;
...
export function css(name, t) {
  const L = lut(name),
    i = Math.max(0, Math.min(255, Math.round(t * 255))) * 3;
...
export function bandColors(K) {
  const out = [];
  for (let c = 0; c < K; c++) out.push(hsl(c / K, 0.75, 0.55));
  return out;
}
