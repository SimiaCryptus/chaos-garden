...
export class Renderer {
  ...
  _buildOrbit() {
    const s = (this.volScene = new THREE.Scene());
    s.background = new THREE.Color(0x05060a);
    this.persp = new THREE.PerspectiveCamera(38, 2, 0.01, 60);
    this.persp.up.set(0, 0, 1);
    this.persp.position.set(2.3, -2.5, 1.6);
    ...
    s.add(
      new THREE.LineSegments(
        ...
        new THREE.LineBasicMaterial({ color: 0x3a4150 })
      )
    );
    // inlet face (−x) outlined in the accent colour so orientation is unambiguous once rotated
    const inlet = new THREE.LineSegments(
      new THREE.EdgesGeometry(new THREE.PlaneGeometry(2 * VOL_HALF.z, 2 * VOL_HALF.y)),
      new THREE.LineBasicMaterial({ color: 0x6cc4ff })
    );
    inlet.rotation.y = Math.PI / 2;
    inlet.position.x = -VOL_HALF.x;
    s.add(inlet);
    this.volMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uVol: { value: null },
        uHalf: { value: VOL_HALF },
        uSteps: { value: 220 },
        uDensity: { value: 14 },
      },
    });
    this.volMesh = new THREE.Mesh(box, this.volMat);
    this.volMesh.renderOrder = 1;
    s.add(this.volMesh);
  }
  setGrid(Nx, Ny) {
    const tex = new THREE.DataTexture(
      new Uint8Array(Nx * Ny * 4),
      Nx,
      Ny,
      THREE.RGBAFormat,
  ...
  setVolumeGrid(Nx, Ny, Nz) {
    ...
    this.volTex?.dispose();
    this.volTex = tex;
    this.volMat.uniforms.uVol.value = tex;
  }
  upload(data) {
  ...
  uploadVolume(data) {
    if (!this.volTex) return;
    this.volTex.image.data.set(data);
  ...
  render() {
    if (this.viewMode === 'orbit') {
      this.controls.update();
      this.renderer.render(this.volScene, this.persp);
