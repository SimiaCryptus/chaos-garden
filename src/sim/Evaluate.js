...
export function* evaluateDesign(job) {
  ...
  const bf = new BarrierField(grid.Nx, grid.Ny);
  bf.resampleFrom(job.mask, job.w, job.h);
  ...
  const settle = (job.settleTime ?? 4) * Tft,
    measure = (job.measureTime ?? 2) * Tft;
  ...
  for (let n = 0; n < total; n++) {
    ...
    twin?.step();
    metrics.tick();
    if (n % 4 === 0) yield { progress: n / (total + 1) };
  }
