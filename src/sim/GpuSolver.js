...
export class GpuSolver extends CpuSolver {
  constructor(grid, params, opts = {}) {
    ...
    if (this.periodicY && grid.Ny & 1)
      throw new Error(`GpuSolver: periodic span requires even Ny (got ${grid.Ny})`);
    const ctx = opts.context || GpuContext.current;
    if (!ctx) throw new Error('WebGPU device unavailable');
    this.ctx = ctx;
    this.device = ctx.device;
    this.wantTracers = false;
    this._dirty = false;
    this._inflight = null;
    this._epoch = 0;
    this._dead = false;
    this._tracersFresh = true; // false once a readback skipped the tracer bodies (only the outlet plane is mirrored then)
    const cx = 1 / (grid.hx * grid.hx),
      cy = 1 / (grid.hy * grid.hy),
      cz = 1 / (grid.hz * grid.hz);
    this._diag = 2 * (cx + cy + cz);
    this._explicit = this.dt * this.nu * this._diag < 0.25;
    this.report.diffusion = this._explicit ? 'explicit' : 'jacobi';
    this.report.residual = 0;
    this._wgN = Math.ceil(grid.N / WG);
    this._wgRows = Math.ceil((grid.Ny * grid.Nz) / WG);
    this._wgOut = Math.ceil((this.K * grid.Ny * grid.Nz) / WG);
  ...
  _alloc(shared) {
    const d = this.device,
      ...
      U = GPUBufferUsage;
    const mk = (size, usage) =>
      d.createBuffer({ size: Math.max(16, Math.ceil(size / 4) * 4), usage });
    ...
    const b = (this.b = {
      ...
      noise: st(4 * M),
      atom: st(8),
      solid: shared ? shared.b.solid : st(4 * N),
      pNb: shared ? shared.b.pNb : st(24 * N),
      pInv: shared ? shared.b.pInv : st(4 * N),
    ...
    this._own = Object.entries(b)
      .filter(([k]) => !(shared && (k === 'solid' || k === 'pNb' || k === 'pInv' || k === 'bodyv')))
      .map(([, v]) => v);
    this._writePass(b.passFwd, 1, 0, 1);
    this._writePass(b.passBwd, -1, 0, 0);
    this._writePass(b.passC0, 0, 0, 0);
    this._writePass(b.passC1, 0, 1, 0);
    this._simBuf = new ArrayBuffer(128);
    this._simU = new Uint32Array(this._simBuf);
    this._simF = new Float32Array(this._simBuf);
    this._pack4 = new Float32Array(4 * N);
    this._packT = K ? new Float32Array(K * N) : null;
  }
  _writePass(buf, dir, color = 0, mode = 0) {
    const ab = new ArrayBuffer(16);
    new Float32Array(ab, 0, 1)[0] = dir;
    new Uint32Array(ab, 4, 2).set([color, mode]);
  ...
  _writeSim() {
    ...
    f[10] = a;
    f[11] = this.sor;
    u[12] = this.periodicY ? 1 : 0;
    u[13] = this.params.inflow === 'parabolic' ? 1 : 0;
    u[14] = this.K;
    u[15] = this.reverse ? 1 : 0;
    f[16] = this.zsT;
    f[17] = 1 / this.dt;
    f[18] = this._diag;
    f[19] = 1 / (1 + a * this._diag);
    f[20] = this.forcing?.amp || 0;
    f[21] = this.forcing?.k || 0;
    f[22] = this._driveF || 0;
    f[23] = 0;
    u[24] = this.fwd ? 1 : 0;
    u[25] = this.torus ? 1 : 0;
  ...
  _bg(pipeline, map) {
    return this.device.createBindGroup({
      ...
      entries: Object.entries(map).map(([k, buffer]) => ({ binding: +k, resource: { buffer } })),
    });
  }
  _build() {
    const d = this.device,
      ...
      mod = this.ctx.module;
    const pl = (entryPoint) =>
      d.createComputePipeline({ layout: 'auto', compute: { module: mod, entryPoint } });
    const P = (this._P = {});
    for (const k of [
      'boundaries',
      'advect',
    ...
    const spec = {
      ...
      combine: [
        P.combine,
        { 0: b.sim, 2: b.vel, 3: b.velA, 4: b.velB, 5: b.mn, 6: b.mx, 7: b.solid, 19: b.bodyv },
      ],
      ...
      sor0: [
        P.sor,
        { 0: b.sim, 1: b.passC0, 9: b.p, 10: b.div, 11: b.pNb, 12: b.pInv, 13: b.atom },
      ],
      sor1: [
        P.sor,
        { 0: b.sim, 1: b.passC1, 9: b.p, 10: b.div, 11: b.pNb, 12: b.pInv, 13: b.atom },
      ],
    ...
    this.pl = {};
    this.bg = {};
    for (const [k, [pipe, map]] of Object.entries(spec)) {
      this.pl[k] = pipe;
      this.bg[k] = this._bg(pipe, map);
    }
  ...
  _uploadGeometry() {
    ...
    const q = this.device.queue,
      b = this.b;
    const s32 = this._solid32 || (this._solid32 = new Uint32Array(this.grid.N));
    s32.set(this.solid);
    q.writeBuffer(b.solid, 0, s32);
    q.writeBuffer(b.pNb, 0, this.pNb);
    q.writeBuffer(b.pInv, 0, this.pInv);
  }
  _pack(a, b, c) {
    const o = this._pack4,
      N = this.grid.N;
    for (let n = 0, m = 0; n < N; n++, m += 4) {
      o[m] = a[n];
      o[m + 1] = b[n];
      o[m + 2] = c ? c[n] : 0;
      o[m + 3] = 0;
  ...
  upload({ tracers = true } = {}) {
    ...
    if (K && tracers) {
      for (let c = 0; c < K; c++) this._packT.set(this.tracers[c], c * N);
      q.writeBuffer(b.tr, 0, this._packT);
      this._tracersFresh = true;
    }
    q.writeBuffer(b.noise, 0, this.inletNoise);
    this._epoch++;
    this._dirty = false; // a readback issued before this upload must not overwrite the mirrors
  }
  setBarriers(bf) {
  ...
  setSolidVelocity(bu2, bv2) {
    ...
    if (this.bodyU)
      this.device.queue.writeBuffer(this.b.bodyv, 0, this._pack(this.bodyU, this.bodyV, null));
    else {
      this._pack4.fill(0);
      this.device.queue.writeBuffer(this.b.bodyv, 0, this._pack4);
    }
  }
  reset(seed = this.params.seed) {
    super.reset(seed);
    if (this.b) this.upload();
  }
  ...
  snapshot() {
    ...
    s.tracersFresh = this._tracersFresh && !this._dirty;
    return s;
  }
  restore(s) {
    super.restore(s);
    const fresh = s.tracersFresh !== false;
    this.upload({ tracers: fresh });
    this._tracersFresh = fresh;
  }
  ...
  copyStateFrom(m) {
    ...
    const enc = this.device.createCommandEncoder(),
      N = this.grid.N,
      M = this.grid.Ny * this.grid.Nz;
    enc.copyBufferToBuffer(m.b.vel, 0, this.b.vel, 0, 16 * N);
    enc.copyBufferToBuffer(m.b.vort, 0, this.b.vort, 0, 16 * N);
    enc.copyBufferToBuffer(m.b.p, 0, this.b.p, 0, 4 * (N + 1));
    enc.copyBufferToBuffer(m.b.noise, 0, this.b.noise, 0, 4 * M);
  ...
  addVelocity(zu, zv, sc) {
    ...
    if (this._dead) return;
    this.device.queue.writeBuffer(this.b.velA, 0, pk);
    this._kernel('addVel', this._wgN);
  }
  ...
  blendToward(other, sc) {
    ...
    if (this._blendFor !== other) {
      this._blendFor = other;
      this._blendBg = this._bg(this._P.blend, {
        0: this.b.sim,
        1: this.b.passScale,
        3: other.b.vel,
        8: this.b.vel,
    ...
    this._writePass(this.b.passScale, sc);
    this._kernel('blend', this._wgN, this._blendBg);
  }
  /** u *= f on the device (mirrors are scaled by the caller). */
  scaleVelocity(f) {
    if (this._dead) return;
    this._writePass(this.b.passScale, f);
    this._kernel('scaleVel', this._wgN);
  }
  ...
  step() {
    ...
    let pass;
    const run = (key, wg = wgN) => {
      pass.setPipeline(this.pl[key]);
      pass.setBindGroup(0, this.bg[key]);
      pass.dispatchWorkgroups(wg);
    };
    pass = enc.beginComputePass();
    ...
    run('combine');
    pass.end();
    enc.copyBufferToBuffer(b.velB, 0, b.vel, 0, 16 * N);
    if (K) {
      pass = enc.beginComputePass();
      run('advectTracers');
      pass.end();
      enc.copyBufferToBuffer(b.trOut, 0, b.tr, 0, 4 * K * N);
    }
    if (this.viscous) {
      pass = enc.beginComputePass();
      if (this._explicit) {
        run('diffuse');
        pass.end();
        enc.copyBufferToBuffer(b.velA, 0, b.vel, 0, 16 * N);
      } else {
        run('jacobi0');
        for (let it = 1; it < this.jacobiIters; it++) run(it & 1 ? 'jacobiA' : 'jacobiB');
        pass.end();
        enc.copyBufferToBuffer(this.jacobiIters & 1 ? b.velA : b.velB, 0, b.vel, 0, 16 * N);
      }
    }
    ...
    for (let it = 0; it < sweeps - 1; it++) {
      run('sor0');
      run('sor1');
    }
    pass.end();
    enc.clearBuffer(b.atom, 4, 4); // residual slot reports the *last* sweep only
    pass = enc.beginComputePass();
    run('sor0');
    run('sor1');
    run('project');
  ...
  sync() {
    ...
    if (this._inflight)
      return this._dirty ? this._inflight.then(() => this.sync()) : this._inflight;
    ...
    const d = this.device,
      ...
      K = this.K,
      M = g.Ny * g.Nz,
      want = this.wantTracers && K > 0;
    const enc = d.createCommandEncoder();
    if (K && !want) {
      const pass = enc.beginComputePass();
      pass.setPipeline(this.pl.gatherOutlet);
      pass.setBindGroup(0, this.bg.gatherOutlet);
      pass.dispatchWorkgroups(this._wgOut);
      pass.end();
    }
    let off = 0;
    const lay = {};
    const cp = (key, buf, bytes) => {
      enc.copyBufferToBuffer(buf, 0, b.stage, off, bytes);
      lay[key] = off;
      off += bytes;
    };
    cp('vel', b.vel, 16 * N);
    ...
    cp('p', b.p, 4 * N);
    cp('div', b.div, 4 * N);
    if (want) cp('tr', b.tr, 4 * K * N);
    else if (K) cp('outlet', b.outlet, 4 * K * M);
    cp('atom', b.atom, 8);
    d.queue.submit([enc.finish()]);
    ...
    this._inflight = b.stage.mapAsync(GPUMapMode.READ, 0, off).then(
      () => {
        try {
          if (epoch === this._epoch && !this._dead)
            this._readback(b.stage.getMappedRange(0, off), lay, want);
        } finally {
          try {
            b.stage.unmap();
          } catch {
  ...
  _readback(ab, lay, want) {
    ...
    o = lay.p >> 2;
    this.p.set(f.subarray(o, o + N));
    o = lay.div >> 2;
    this.div.set(f.subarray(o, o + N));
    if (want) {
      o = lay.tr >> 2;
      for (let c = 0; c < K; c++) this.tracers[c].set(f.subarray(o + c * N, o + (c + 1) * N));
    } else if (K) {
      // only the outlet plane (what MutualInfo reads) — the rest of the tracer mirrors stay stale
      o = lay.outlet >> 2;
      const M = Ny * Nz,
        oc = this.outCol;
      for (let c = 0; c < K; c++) {
        const T = this.tracers[c];
        for (let k = 0; k < Nz; k++)
          for (let j = 0; j < Ny; j++) T[oc + j * Nx + k * Nx * Ny] = f[o + c * M + j + Ny * k];
      }
    }
    ...
    const r = this.report;
    r.divMax = a[0];
    r.divNorm = (a[0] * g.hx) / this.Uref;
    r.residual = a[1];
  }
  dispose() {
    this._dead = true;
    for (const buf of this._own) {
      try {
        buf.destroy();
      } catch {
        /* ignore */
