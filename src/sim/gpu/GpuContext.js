...
export class GpuContext {
  ...
  static async init({ force = false } = {}) {
    ...
    let flag = null;
    try {
      flag = sessionStorage.getItem(NOGPU);
    } catch {
      /* no storage */
    }
    ...
    try {
      ...
      const errors = ci.messages
        .filter((m) => m.type === 'error')
        .map((m) => `${m.lineNum}:${m.linePos} ${m.message}`);
      if (errors.length) {
        log.error('webgpu shader errors', errors);
        device.destroy();
        return off('shader compile error');
      }
      const ctx = new GpuContext(adapter, device, module, info);
      device.lost.then((e) => ctx.fail(`device lost (${e.reason}): ${e.message}`));
      device.addEventListener('uncapturederror', (e) =>
        ctx.fail(e.error?.message || String(e.error))
      );
      try {
        sessionStorage.removeItem(NOGPU);
      } catch {
        /* ignore */
      }
      ...
      log.warn('webgpu init failed', String(err));
      return off(String(err?.message || err));
    }
  }
  ...
  get label() {
    return [this.info.vendor, this.info.architecture].filter(Boolean).join('/') || 'webgpu';
  }
  fail(reason) {
    ...
    try {
      sessionStorage.setItem(NOGPU, reason);
    } catch {
      /* ignore */
    }
