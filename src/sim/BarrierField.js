...
export function edt2d(mask, w, h, pred) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) f[x] = g[x + y * w];
    edt1d(f, w, d, v, z);
    for (let x = 0; x < w; x++) g[x + y * w] = d[x];
  }
  return g;
}
...
export class BarrierField {
  constructor(Nx, Ny) {
    ...
    this.version = 0;
    this.protectedCols = 3;
  }
  get(i, j) {
    return this.mask[i + j * this.Nx];
  }
  set(i, j, v) {
    if (i < 0 || j < 0 || i >= this.Nx || j >= this.Ny) return;
    this.mask[i + j * this.Nx] = v ? 1 : 0;
  }
  paintable(i) {
    return i >= this.protectedCols && i < this.Nx - this.protectedCols;
  }
  bump() {
    this.version++;
  }
  ...
  count() {
    let c = 0;
    for (let n = 0; n < this.mask.length; n++) c += this.mask[n];
    return c;
  }
  fraction() {
    ...
    if (w === this.Nx && h === this.Ny) this.mask.set(mask);
      ...
      for (let j = 0; j < this.Ny; j++)
        for (let i = 0; i < this.Nx; i++) {
          const si = Math.min(w - 1, Math.floor(((i + 0.5) / this.Nx) * w)),
            sj = Math.min(h - 1, Math.floor(((j + 0.5) / this.Ny) * h));
  ...
  geometricScales(hy) {
    ...
    const toSolid = edt2d(this.mask, w, h, (m) => m === 1),
      toFluid = edt2d(this.mask, w, h, (m) => m === 0);
    const ridge = (d, want) => {
      const vals = [];
      for (let j = 1; j < h - 1; j++)
        for (let i = this.protectedCols; i < w - this.protectedCols; i++) {
          const n = i + j * w;
          if ((this.mask[n] === 1) !== want) continue;
          const c = d[n];
          if (c <= 0 || c >= BIG / 2) continue;
          if (c >= d[n - 1] && c >= d[n + 1] && c >= d[n - w] && c >= d[n + w]) {
            const l = 2 * Math.sqrt(c) * hy;
            if (l < 0.6) vals.push(l);
          }
        }
      return peaks(vals);
    };
...
function peaks(vals, bins = 24, lo = 0.02, hi = 1) {
  ...
  for (const v of vals) {
    const b = Math.floor(
      ((Math.log(Math.max(lo, Math.min(hi, v))) - llo) / (lhi - llo)) * (bins - 1)
    );
    hist[b]++;
  }
  const out = [];
  ...
  for (let b = 0; b < bins; b++)
    if (
      hist[b] >= 0.25 * max &&
      hist[b] >= (b > 0 ? hist[b - 1] : 0) &&
      hist[b] >= (b < bins - 1 ? hist[b + 1] : 0)
    )
      out.push({
        scale: Math.exp(llo + ((b + 0.5) / (bins - 1)) * (lhi - llo)),
        weight: hist[b] / max,
      });
