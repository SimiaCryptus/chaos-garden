...
export function relDiff(solver, snap) {
  let num = 0,
    den = 0;
  for (let n = 0; n < solver.grid.N; n++) {
    if (solver.solid[n]) continue;
    const du = solver.u[n] - snap.u[n],
      dv = solver.v[n] - snap.v[n],
      dw = solver.w[n] - snap.w[n];
    num += du * du + dv * dv + dw * dw;
    den += snap.u[n] ** 2 + snap.v[n] ** 2 + snap.w[n] ** 2;
  }
...
export function* rewindProbe(solver, N = 200) {
  ...
  for (let n = 0; n < N; n++) {
    solver.step();
    if (n % 8 === 0) {
      yield { phase: 'forward', progress: n / (2 * N) };
      yield* settle(solver);
    }
  }
  yield* settle(solver);
  negate(solver);
  const visc = solver.viscous;
  solver.reverse = true;
  solver.viscous = false;
  for (let n = 0; n < N; n++) {
    solver.step();
    if (n % 8 === 0) {
      yield { phase: 'reverse', progress: 0.5 + n / (2 * N) };
      yield* settle(solver);
    }
  }
  yield* settle(solver);
  ...
  return {
    D_rev,
    N,
    horizonTime: N * solver.dt,
    flowThroughs: (N * solver.dt) / (solver.grid.Lx / solver.Uref),
  };
}
export function runRewind(solver, N) {
  const g = rewindProbe(solver, N);
  let r;
  do r = g.next();
  while (!r.done);
  return r.value;
