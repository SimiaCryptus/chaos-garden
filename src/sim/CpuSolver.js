...
export class CpuSolver {
  constructor(grid, params, opts = {}) {
    this.grid = grid;
    this.params = { ...params };
    this.U0 = params.flow ?? 1;
    this.nu = 1 / params.Re; // U₀ is signed: negative flow enters through the right face
    this.torus = params.topology === 'torus';
    this.driveTau = 0.25; // torus: the mean flow is relaxed toward U₀ on this time scale
    this.bodyU = null;
    this.bodyV = null; // velocity carried by solid cells (moving rigid bodies); null ⇒ walls at rest
    this.pIters = opts.pIters ?? 40;
    this.sor = opts.sor ?? 1.5;
    this.jacobiIters = 16;
    this.pTol = opts.pTol ?? 1e-6; // SOR stops when max|Δp| in a sweep drops below this (state-dependent ⇒ deterministic, §5.4)
    this.cfl = 0.5;
    this.dt = (this.cfl * Math.min(grid.hx, grid.hy)) / this.Uref;
    ...
    this.pNb = new Int32Array(6 * N);
    this.pInv = new Float32Array(N);
    this._buildPressureCoeffs();
    this.tracers = [];
    this.tracerTmp = [];
    for (let c = 0; c < this.K; c++) {
      this.tracers.push(f());
      this.tracerTmp.push(f());
    }
  ...
  profile(k) {
    const z = (k + 0.5) / this.grid.Nz;
    return this.params.inflow === 'parabolic' ? 6 * z * (1 - z) : 1;
  }
  ...
  get outCol() {
    return this.fwd ? this.grid.Nx - 2 : 1;
  }
  /** Boundary column that carries the inlet profile (and the dye bands). */
  get inCol() {
    return this.fwd ? 0 : this.grid.Nx - 1;
  }
  /**
  ...
  setBarriers(bf) {
    ...
    const bu = this.bodyU,
      bv = this.bodyV;
    for (let n = 0; n < this.grid.N; n++)
      if (this.solid[n]) {
        any = 1;
        this.u[n] = bu ? bu[n] : 0;
        this.v[n] = bv ? bv[n] : 0;
        this.w[n] = this.p[n] = 0;
        for (const T of this.tracers) T[n] = 0;
      }
    this.hasSolid = !!any;
    this.barrierVersion = bf.version;
    this._buildPressureCoeffs();
  }
  /**
  ...
  setSolidVelocity(bu2, bv2) {
    const g = this.grid;
    if (!bu2) {
      this.bodyU = this.bodyV = null;
      return;
    }
    if (!this.bodyU) {
      this.bodyU = new Float32Array(g.N);
      this.bodyV = new Float32Array(g.N);
    }
    for (let k = 0; k < g.Nz; k++) {
      this.bodyU.set(bu2, k * g.sz);
      this.bodyV.set(bv2, k * g.sz);
    }
  }
  /**
    ...
    for (let k = 0; k < Nz; k++)
      for (let j = 0; j < Ny; j++)
        for (let i = 1; i < Nx - 1; i++) {
          ...
          const jm = j === 0 ? (per ? n + (Ny - 1) * sy : -1) : n - sy,
            jp = j === Ny - 1 ? (per ? n - (Ny - 1) * sy : -1) : n + sy;
          if (jm >= 0 && !solid[jm]) {
            nb[m + 2] = jm;
            d += cy;
          }
  ...
  reset(seed = this.params.seed) {
    ...
    for (let k = 0; k < g.Nz; k++) {
      const pr = this.profile(k);
      for (let j = 0; j < g.Ny; j++)
        for (let i = 0; i < g.Nx; i++) {
          const n = g.idx(i, j, k);
          if (this.solid[n]) {
            this.u[n] = this.bodyU ? this.bodyU[n] : 0;
            this.v[n] = this.bodyV ? this.bodyV[n] : 0;
            this.w[n] = 0;
            continue;
          }
          this.u[n] = this.U0 * pr * (1 + amp * (rng.next() - 0.5));
          this.v[n] = amp * (rng.next() - 0.5);
          this.w[n] = 0;
        }
    }
    this.p.fill(0);
    for (const T of this.tracers) T.fill(0);
    this.t = 0;
    this.stepCount = 0;
    ...
    for (let k = 0; k < Nz; k++) {
      ...
      for (let j = 0; j < Ny; j++) {
        ...
        const inl = fwd ? i0 : i1,
          out = fwd ? i1 : i0,
          nbr = fwd ? i1 - 1 : i0 + 1,
          noise = amp * U * pr * this.inletNoise[j + Ny * k];
        if (torus) {
          // periodic ghosts: column 0 ≡ Nx−2, column Nx−1 ≡ 1
          u[i0] = u[i1 - 1];
          v[i0] = v[i1 - 1];
          w[i0] = w[i1 - 1];
          u[i1] = u[i0 + 1];
          ...
          // rewind probe: both faces carry the negated inflow
          u[inl] = -U * pr;
          v[inl] = noise;
          w[inl] = 0;
          u[out] = -U * pr;
  ...
  sample(f, x, y, z, zs, ys, mm) {
    ...
    const b00 = i0 + ja * sy + ka * sz,
      b10 = i0 + jb * sy + ka * sz,
      b01 = i0 + ja * sy + kb * sz,
      b11 = i0 + jb * sy + kb * sz;
    const c000 = f[b00] * ra * sa,
      c100 = f[b00 + 1] * ra * sa,
      c010 = f[b10] * rb * sa,
      c110 = f[b10 + 1] * rb * sa;
    const c001 = f[b01] * ra * sb,
      c101 = f[b01 + 1] * ra * sb,
      c011 = f[b11] * rb * sb,
      c111 = f[b11 + 1] * rb * sb;
    if (mm) {
      ...
      if (c100 < lo) lo = c100;
      else if (c100 > hi) hi = c100;
      if (c010 < lo) lo = c010;
      else if (c010 > hi) hi = c010;
      if (c110 < lo) lo = c110;
      else if (c110 > hi) hi = c110;
      if (c001 < lo) lo = c001;
      else if (c001 > hi) hi = c001;
      if (c101 < lo) lo = c101;
      else if (c101 > hi) hi = c101;
      if (c011 < lo) lo = c011;
      else if (c011 > hi) hi = c011;
      if (c111 < lo) lo = c111;
      else if (c111 > hi) hi = c111;
      this._mm[0] = lo;
      this._mm[1] = hi;
    }
    const x00 = c000 + tx * (c100 - c000),
      x10 = c010 + tx * (c110 - c010),
      x01 = c001 + tx * (c101 - c001),
      x11 = c011 + tx * (c111 - c011);
  ...
  _advectVel(SU, SV, SW, OU, OV, OW, dt, dir, mm) {
    ...
    const u = this.u,
      ...
      bu = this.bodyU,
      bv = this.bodyV;
    const ax = (dt * dir) / g.hx,
      ay = (dt * dir) / g.hy,
      az = (dt * dir) / g.hz;
    const [mn0, mn1, mn2] = this.mn,
      [mx0, mx1, mx2] = this.mx;
    for (let k = 0; k < Nz; k++)
      for (let j = 0; j < Ny; j++) {
        const row = j * sy + k * sz,
        ...
        for (let i = 1; i < Nx - 1; i++) {
          ...
          if (solid[n]) {
            ...
            OW[n] = 0;
            continue;
          } // solids keep the body's velocity through the step
          const x = i + 0.5,
            y = j + 0.5,
            z = k + 0.5;
          const xm = x - 0.5 * ax * u[n],
            ym = y - 0.5 * ay * v[n],
            zm = z - 0.5 * az * w[n];
          const um = this.sample(u, xm, ym, zm, zsT, 1, false),
            vm = this.sample(v, xm, ym, zm, zsT, -1, false),
            wm = this.sample(w, xm, ym, zm, -1, 1, false);
          const xb = x - ax * um,
            yb = y - ay * vm,
            zb = z - az * wm;
          OU[n] = this.sample(SU, xb, yb, zb, zsT, 1, mm);
          if (mm) {
            mn0[n] = this._mm[0];
            mx0[n] = this._mm[1];
          }
          OV[n] = this.sample(SV, xb, yb, zb, zsT, -1, mm);
          if (mm) {
            mn1[n] = this._mm[0];
            mx1[n] = this._mm[1];
          }
          OW[n] = this.sample(SW, xb, yb, zb, -1, 1, mm);
          if (mm) {
            mn2[n] = this._mm[0];
            mx2[n] = this._mm[1];
          }
    ...
    for (let k = 0; k < Nz; k++)
      for (let j = 0; j < Ny; j++) {
        ...
        for (let i = 1; i < Nx - 1; i++) {
          ...
          const b00 = i0 + ja * sy + k0 * sz,
            b10 = i0 + jb * sy + k0 * sz,
            b01 = b00 + sz,
            b11 = b10 + sz;
          for (let c = 0; c < K; c++) {
            const f = T[c];
            const x00 = f[b00] + tx * (f[b00 + 1] - f[b00]),
              x10 = f[b10] + tx * (f[b10 + 1] - f[b10]);
            const x01 = f[b01] + tx * (f[b01 + 1] - f[b01]),
              x11 = f[b11] + tx * (f[b11 + 1] - f[b11]);
            const y0 = x00 + ty * (x10 - x00),
              y1 = x01 + ty * (x11 - x01);
            O[c][n] = y0 + tz * (y1 - y0);
          }
  ...
  _nbSum(f, i, j, k, n, zs, ys) {
    ...
    if (this.periodicY)
      s +=
        cy * (f[n + (j === 0 ? (Ny - 1) * sy : -sy)] + f[n + (j === Ny - 1 ? -(Ny - 1) * sy : sy)]);
  ...
  _diffuse(dt) {
    const g = this.grid,
      ...
      a = dt * this.nu,
      solid = this.solid;
    const cx = (this._cx = 1 / (g.hx * g.hx)),
      cy = (this._cy = 1 / (g.hy * g.hy)),
      cz = (this._cz = 1 / (g.hz * g.hz)),
      diag = 2 * (cx + cy + cz);
    const comps = [
      [this.u, this.ua, this.ub, this.zsT, 1],
      [this.v, this.va, this.vb, this.zsT, -1],
      [this.w, this.wa, this.wb, -1, 1],
    ];
    if (a * diag < 0.25) {
        ...
        for (let k = 0; k < Nz; k++)
          for (let j = 0; j < Ny; j++) {
            ...
            for (let i = 1; i < Nx - 1; i++) {
              const n = row + i;
              if (solid[n]) continue;
              out[n] = f[n] + a * (this._nbSum(f, i, j, k, n, zs, ys) - diag * f[n]);
            }
          }
        f.set(out);
      }
          ...
          for (let k = 0; k < Nz; k++)
            for (let j = 0; j < Ny; j++) {
              ...
              for (let i = 1; i < Nx - 1; i++) {
                const n = row + i;
                if (solid[n]) continue;
                xn[n] = (rhs[n] + a * this._nbSum(x, i, j, k, n, zs, ys)) * inv;
              }
            }
          const tmp = x;
          x = xn;
  ...
  _driveForce() {
    ...
    const target = this.reverse ? -this.U0 : this.U0,
      f = (target - s / Math.max(1, c)) / this.driveTau;
    this.report.drive = f;
    return f;
  }
  _addForces(dt) {
    const g = this.grid,
      ...
      v = this.v;
    if (this.torus) {
      const f = dt * this._driveForce();
      for (let n = 0; n < N; n++) if (!solid[n]) u[n] += f;
    }
    if (this.forcing) {
      ...
      for (let n = 0; n < N; n++) {
        if (solid[n]) continue;
        const i = n % g.Nx,
          j = Math.floor(n / g.Nx) % g.Ny;
        u[n] += dt * amp * Math.sin((2 * Math.PI * k * (j + 0.5)) / g.Ny);
        v[n] += dt * amp * Math.sin(((2 * Math.PI * k * (i + 0.5)) / g.Nx) * 2);
      }
    }
  }
  _divergence(out) {
        ...
        for (let k = 0; k < Nz; k++)
          for (let j = 0; j < Ny; j++) {
            ...
            for (let i = 1 + ((1 + j + k + color) & 1); i < Nx - 1; i += 2) {
              const n = row + i,
                id = inv[n];
              if (id === 0) continue; // solid or fully enclosed
              const m = 6 * n;
              const s =
                cx * (p[nb[m]] + p[nb[m + 1]]) +
                cy * (p[nb[m + 2]] + p[nb[m + 3]]) +
                cz * (p[nb[m + 4]] + p[nb[m + 5]]);
              const d = om * ((s - rhs[n] * idt) * id - p[n]);
              p[n] += d;
              const ad = d < 0 ? -d : d;
              if (ad > maxUp) maxUp = ad;
            }
          }
      if (maxUp < tol) {
        it++;
    ...
    for (let k = 0; k < Nz; k++)
      for (let j = 0; j < Ny; j++) {
        ...
        for (let i = 1; i < Nx - 1; i++) {
          ...
          if (i === 1) pxm = torus ? (solid[n + wrap] ? pc : p[n + wrap]) : fwd ? pc : 0;
          else pxm = solid[n - 1] ? pc : p[n - 1];
          if (i === Nx - 2) pxp = torus ? (solid[n - wrap] ? pc : p[n - wrap]) : fwd ? 0 : pc;
          else pxp = solid[n + 1] ? pc : p[n + 1];
          let pym, pyp;
          if (per) {
            const jm = n + (j === 0 ? (Ny - 1) * sy : -sy),
              jp = n + (j === Ny - 1 ? -(Ny - 1) * sy : sy);
            pym = solid[jm] ? pc : p[jm];
            pyp = solid[jp] ? pc : p[jp];
          } else {
            pym = j > 0 && !solid[n - sy] ? p[n - sy] : pc;
            pyp = j < Ny - 1 && !solid[n + sy] ? p[n + sy] : pc;
          }
          const pzm = k > 0 && !solid[n - sz] ? p[n - sz] : pc,
            pzp = k < Nz - 1 && !solid[n + sz] ? p[n + sz] : pc;
          u[n] -= ax * (pxp - pxm);
          v[n] -= ay * (pyp - pym);
          w[n] -= az * (pzp - pzm);
        }
      ...
      for (let j = 0; j < Ny; j++)
        for (let i = 0; i < Nx; i++) {
          ...
          if (per) {
            ...
            uN = j === Ny - 1 ? u[n] : u[n + sy];
            uS = j === 0 ? u[n] : u[n - sy];
            wN = j === Ny - 1 ? w[n] : w[n + sy];
            wS = j === 0 ? w[n] : w[n - sy];
          }
          const uT = k === Nz - 1 ? zs * u[n] : u[n + sz],
            uB = k === 0 ? zs * u[n] : u[n - sz];
          const vT = k === Nz - 1 ? zs * v[n] : v[n + sz],
            vB = k === 0 ? zs * v[n] : v[n - sz];
          const dudy = (uN - uS) * iy,
            dwdy = (wN - wS) * iy,
            dudz = (uT - uB) * iz,
            dvdz = (vT - vB) * iz;
  ...
  step() {
    ...
    const { u, v, w, ua, va, wa, ub, vb, wb, solid } = this,
      [mn0, mn1, mn2] = this.mn,
      [mx0, mx1, mx2] = this.mx;
    for (let row = 0; row < N; row += Nx)
      for (let n = row + 1; n < row + Nx - 1; n++) {
        // rows are contiguous in x: no modulo per cell
        if (solid[n]) continue;
        let c = ua[n] + 0.5 * (u[n] - ub[n]);
        ua[n] = c < mn0[n] ? mn0[n] : c > mx0[n] ? mx0[n] : c;
        c = va[n] + 0.5 * (v[n] - vb[n]);
        va[n] = c < mn1[n] ? mn1[n] : c > mx1[n] ? mx1[n] : c;
        c = wa[n] + 0.5 * (w[n] - wb[n]);
        wa[n] = c < mn2[n] ? mn2[n] : c > mx2[n] ? mx2[n] : c;
      }
    this.u = ua;
    this.ua = u;
    this.v = va;
    ...
    r.t = this.t;
    r.divMax = divMax;
    r.divNorm = (divMax * g.hx) / this.Uref; // r.pIters is set by _pressure
    return r;
  }
  snapshot() {
    return {
      ...
      u: Float32Array.from(this.u),
      v: Float32Array.from(this.v),
      w: Float32Array.from(this.w),
      p: Float32Array.from(this.p),
      tracers: this.tracers.map((T) => Float32Array.from(T)),
  ...
  restore(s) {
    ...
    this.w.set(s.w);
    this.p.set(s.p);
    s.tracers.forEach((T, c) => this.tracers[c]?.set(T));
