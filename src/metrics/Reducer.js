...
export function bulkInvariants(solver) {
  const g = solver.grid,
    ...
    sy = Nx,
    sz = Nx * Ny;
  const { u, v, w, wx, wy, wz, solid } = solver,
    nu = solver.nu;
  const ix = 0.5 / g.hx,
    iy = 0.5 / g.hy,
    iz = 0.5 / g.hz;
  ...
  const div = solver.div,
    oc = solver.outCol ?? Nx - 2,
    Uref = solver.Uref ?? Math.abs(solver.U0); // outlet column follows the sign of U₀
  // |∇f|² at cell m (central differences); a plain function so the hot loop allocates nothing.
  const g3 = (f, m) => {
    const gx = (f[m + 1] - f[m - 1]) * ix,
      gy = (f[m + sy] - f[m - sy]) * iy,
      gz = (f[m + sz] - f[m - sz]) * iz;
    ...
    for (let j = 0; j < Ny; j++)
      for (let i = 0; i < Nx; i++) {
        ...
        const uu = u[m],
          ...
          ww = w[m];
        E += uu * uu + vv * vv + ww * ww;
        Z += wx[m] * wx[m] + wy[m] * wy[m] + wz[m] * wz[m];
        const sp = Math.abs(uu) + Math.abs(vv) + Math.abs(ww);
        if (sp > umax) umax = sp;
        const ad = Math.abs(div[m]);
  ...
  return {
    ...
    eps: ng ? (nu * eps) / ng : 0,
    divMax,
    divNorm: (divMax * g.hx) / Uref,
    umax,
    cfl: (umax * solver.dt) / g.hx,
    Q: nq && solver.U0 ? Q / nq / solver.U0 : 0,
  };
}
