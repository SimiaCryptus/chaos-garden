...
export class MetricsSuite {
  constructor(solver, twin = null, opts = {}) {
    ...
    this.twin = twin;
    this.settleTime =
      opts.settleTime ?? (4 * solver.grid.Lx) / (solver.Uref ?? Math.abs(solver.U0)); // flow-throughs, independent of the flow direction
  ...
  reset() {
    ...
    this.cnt = {};
    this.avg = {};
    this.tStart = this.solver.t;
    this.mi.reset();
    if (this.lyap)
      this.lyap.init(this.solver, this.twin, new Rng((this.solver.params.seed ^ 0xa5a5a5a5) >>> 0));
  }
  get settled() {
    return this.solver.t - this.tStart >= this.settleTime;
  }
  get settleProgress() {
    return Math.min(1, (this.solver.t - this.tStart) / this.settleTime);
  }
  /**
  ...
  tick(steps = 1) {
    ...
    if (crossed(this.cadence.spectra)) {
      this.spectra = computeSpectra(this.solver, this.win);
      this.spectra.breadth = breadthFromFlux(this.spectra.k, this.spectra.Pi);
    }
    if (crossed(this.cadence.mi)) this.miValue = this.mi.compute();
    if (this.settled && crossed(this.cadence.average)) {
      this._acc('lambda', this.lyap?.lambda);
      this._acc('Hw', this.entropy?.Hw);
      this._acc('Hang', this.entropy?.Hang);
      this._acc('I', this.miValue);
      this._acc('breadth', this.spectra?.breadth);
      this._acc('Q', this.bulk.Q);
    }
  }
  _acc(k, v) {
    ...
    this.avg[k] = this.sum[k] / this.cnt[k];
  }
  setRewind(D) {
    this.rewind = D;
  }
  raw() {
    ...
    return {
      ...
      Hw: a.Hw ?? this.entropy?.Hw ?? null,
      Hang: a.Hang ?? this.entropy?.Hang ?? null,
      I: a.I ?? this.miValue ?? null,
      Drev: this.rewind,
      breadth: a.breadth ?? this.spectra?.breadth ?? null,
      Q: a.Q ?? b.Q,
      divNorm: b.divNorm,
      divMax: b.divMax,
      P: b.P,
      ...
      eps: b.eps,
      cfl: b.cfl,
      hx: this.solver.grid.hx,
    };
  }
  currentScore() {
    return composite(this.raw(), { settled: this.settled });
  }
  summary() {
    const raw = this.raw(),
      res = composite(raw, { settled: this.settled }),
      s = this.spectra;
    return {
      raw,
      ...res,
