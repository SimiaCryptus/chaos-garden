...
export const WEIGHTS = { lyapunov: 0.25, entropy: 0.15, infoLoss: 0.2, rewind: 0.2, breadth: 0.2 };
...
export const DEFINITIONS = {
  lyapunov:
    'Finite-time Lyapunov exponent λ from a twin solver perturbed by δ₀=1e-6, renormalized every T_r=32 steps; normalized by ' +
    NORMALIZERS.lyapunov +
    ' U₀/Ly.',
  entropy:
    'Geometric mean of the Shannon entropy of log|ω| (256 bins, fixed range) and of the velocity-direction histogram (64 bins), analysis window only.',
  infoLoss:
    '1 − Î, where Î is inlet-band→outlet-band mutual information (16 bands, flux-weighted, bits / log₂16).',
  rewind:
    'D_rev = ‖u_rewound − u₀‖₂/‖u₀‖₂ after N forward steps, velocity negation with 1/Re→0, and N reversed steps.',
  breadth:
    'Decades of wavenumber over which |Π(k)| exceeds 10% of its maximum, plus 0.5 when forward and inverse flux coexist (split cascade).',
};
export function breadthFromFlux(k, Pi) {
  let m = 0;
  for (let i = 1; i < Pi.length; i++) m = Math.max(m, Math.abs(Pi[i]));
  if (!(m > 0)) return 0;
  let kmin = Infinity,
    kmax = 0,
    cnt = 0,
  ...
  for (let i = 1; i < Pi.length; i++)
    if (Math.abs(Pi[i]) > 0.1 * m) {
      cnt++;
      kmin = Math.min(kmin, k[i]);
...
export function composite(raw, opts = {}) {
  const N = NORMALIZERS,
    parts = {},
    c01 = (x) => (x < 0 ? 0 : x > 1 ? 1 : x),
    fin = (x) => x != null && Number.isFinite(x);
  if (fin(raw.lambda)) parts.lyapunov = c01(raw.lambda / N.lyapunov);
  if (fin(raw.Hw))
    parts.entropy = c01(
      Math.sqrt(Math.max(0, raw.Hw) * Math.max(0, fin(raw.Hang) ? raw.Hang : raw.Hw)) / N.entropy
    );
  ...
  let wsum = 0;
  for (const k in parts) wsum += WEIGHTS[k];
  const weights = {};
  let s = 0;
  for (const k in parts) {
    weights[k] = wsum > 0 ? WEIGHTS[k] / wsum : 0;
    s += weights[k] * parts[k];
  }
  const Q = fin(raw.Q) ? raw.Q : 1,
    gate = Math.min(1, Q / N.Qmin);
  const flags = [];
  ...
  if (
    ...
    raw.Z > 0 &&
    fin(raw.hx) &&
    (raw.P * raw.hx * raw.hx) / raw.Z > N.resolutionRatio
  )
    flags.push('unresolved');
  if (Q < N.Qmin) flags.push('lowThroughput');
  return {
    score: wsum > 0 ? 100 * s * gate : 0,
    parts,
    weights,
    gate,
    flags,
    valid: !flags.includes('diverged') && !flags.includes('settling'),
