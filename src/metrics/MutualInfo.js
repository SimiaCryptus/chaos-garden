...
export class MutualInfo {
  constructor(K) {
    this.K = K;
    this.joint = new Float64Array(K * K);
    this.forget = 0.998;
  }
  reset() {
    this.joint.fill(0);
  ...
  accumulate(solver, steps = 1) {
    ...
    const g = solver.grid,
      Nx = g.Nx,
      Ny = g.Ny,
      i = solver.outCol ?? Nx - 2,
      sgn = solver.fwd === false ? -1 : 1,
      sy = Nx,
      sz = Nx * Ny,
      J = this.joint;
    ...
    for (let k = 0; k < g.Nz; k++)
      for (let j = 0; j < Ny; j++) {
        ...
        const flux = solver.u[idx] * sgn * steps;
        if (flux <= 0) continue;
  ...
  compute() {
    const K = this.K,
      J = this.joint;
    let tot = 0;
    for (let n = 0; n < J.length; n++) tot += J[n];
    if (!(tot > 0)) return null;
    const pi = new Float64Array(K),
      po = new Float64Array(K);
    for (let c = 0; c < K; c++)
      for (let o = 0; o < K; o++) {
        const p = J[c * K + o] / tot;
        pi[c] += p;
    ...
    for (let c = 0; c < K; c++)
      for (let o = 0; o < K; o++) {
        const p = J[c * K + o] / tot;
        if (p > 0) I += p * Math.log2(p / (pi[c] * po[o]));
      }
